loadstring(game:HttpGet("https://raw.githubusercontent.com/lyquyenanh21-glitch/Anh-lyhud/refs/heads/main/hackphu2.lua"))()

-- ==========================================
-- AUTO FARM LEVEL - FULL SEA 1-2-3 (MAX 2800)
-- ==========================================
...
-- Tự động cộng chỉ số Melee và Phòng thủ
spawn(function()
    while wait(1) do
        pcall(function()
        ...
        pcall(function()
            ...
            for _, src in pairs(sources) do
                for _, t in pairs(src:GetChildren()) do
                    if t:IsA("Tool") then
                        if ChooseWeapon == "Melee" and t.ToolTip == "Melee" then SelectWeapon = t.Name end
                    end
                end
            end
        end)
...
-- Mở rộng CheckLevel cho SEA 2 và SEA 3
local oldCheckLevel = CheckLevel
function CheckLevel()
    oldCheckLevel()
    local lvl = plr.Data.Level.Value

    -- SEA 2
    if lvl > 699 and lvl <= 724 then
    ...
    elseif lvl > 724 and lvl <= 774 then
    ...
    elseif lvl > 774 and lvl <= 799 then
    ...
    elseif lvl > 799 and lvl <= 874 then
    ...
    elseif lvl > 874 and lvl <= 899 then
    ...
    elseif lvl > 899 and lvl <= 949 then
    ...
    elseif lvl > 949 and lvl <= 974 then
    ...
    elseif lvl > 974 and lvl <= 999 then
    ...
    elseif lvl > 999 and lvl <= 1049 then
    ...
    elseif lvl > 1049 and lvl <= 1099 then
    ...
    elseif lvl > 1099 and lvl <= 1124 then
    ...
    elseif lvl > 1124 and lvl <= 1174 then
    ...
    elseif lvl > 1174 and lvl <= 1199 then
    ...
    elseif lvl > 1199 and lvl <= 1249 then
    ...
    elseif lvl > 1249 and lvl <= 1274 then
    ...
    elseif lvl > 1274 and lvl <= 1299 then
        ...
        pcall(function() game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(923.212, 126.976, 32852.832)) end)
    elseif lvl > 1299 and lvl <= 1324 then
        ...
        pcall(function() game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(923.212, 126.976, 32852.832)) end)
    elseif lvl > 1324 and lvl <= 1349 then
        ...
        pcall(function() game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(923.212, 126.976, 32852.832)) end)
    elseif lvl > 1349 and lvl <= 1374 then
    ...
    elseif lvl > 1374 and lvl <= 1424 then
    ...
    elseif lvl > 1424 and lvl <= 1449 then
    ...
    elseif lvl > 1449 and lvl <= 1499 then
...
-- ==========================================
-- ANHLY HUD - Hiển thị Level, Beli, Point, Bounty
-- ==========================================

local function CreateHUD()
    local PlayerGui = plr:WaitForChild("PlayerGui")
    
    if PlayerGui:FindFirstChild("AnhLyHUD") then
        PlayerGui.AnhLyHUD:Destroy()
    end
    
    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "AnhLyHUD"
    ScreenGui.Parent = PlayerGui
    ScreenGui.ResetOnSpawn = false
    
    local MainHUD = Instance.new("Frame")
    MainHUD.Name = "MainHUD"
    MainHUD.Size = UDim2.new(0, 220, 0, 130)
    MainHUD.Position = UDim2.new(1, -230, 0, 10)
    ...
    TitleBar.BackgroundColor3 = Color3.fromRGB(0, 180, 0)
    TitleBar.BorderSizePixel = 0
    TitleBar.Parent = MainHUD
    ...
    InfoContainer.Size = UDim2.new(1, -20, 1, -35)
    InfoContainer.Position = UDim2.new(0, 10, 0, 33)
    InfoContainer.BackgroundTransparency = 1
    InfoContainer.Parent = MainHUD
    
    local function CreateInfoLine(name, icon, yPos)
        ...
        ValueLabel.Size = UDim2.new(1, -95, 1, 0)
        ValueLabel.Position = UDim2.new(0, 92, 0, 0)
        ValueLabel.Text = "0"
    ...
    local LevelValue = CreateInfoLine("Level", "⬆️", 0)
    local BeliValue = CreateInfoLine("Beli", "💰", 23)
    local PointValue = CreateInfoLine("Point", "⭐", 46)
    local BountyValue = CreateInfoLine("Bounty", "💀", 69)
    
    spawn(function()
        while wait(0.5) do
            pcall(function()
                if plr and plr.Data then
                    LevelValue.Text = tostring(plr.Data.Level.Value)
                    BeliValue.Text = tostring(plr.Data.Beli.Value)
                    PointValue.Text = tostring(plr.Data.Stats.Points.Value)
                    if plr.leaderstats and plr.leaderstats:FindFirstChild("Bounty") then
                        BountyValue.Text = tostring(plr.leaderstats.Bounty.Value)
                    end
                end
            end)
        end
