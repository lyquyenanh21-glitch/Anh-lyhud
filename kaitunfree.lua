...
loadstring(game:HttpGet("https://pastefy.app/9oujkvIJ/raw"))()

-- ==========================================
-- AUTO FARM LEVEL - TỰ ĐỘNG CHUYỂN SEA + AUTO STATS + TỘC V2/V3
-- ==========================================

local Player = game:GetService("Players").LocalPlayer
local PlayerGui = Player:WaitForChild("PlayerGui")

if PlayerGui:FindFirstChild("Status_UI") then PlayerGui["Status_UI"]:Destroy() end
...
StatusText.Text = "🤖 Status: Auto Farming..."
    ...
    while task.wait(0.5) do
        pcall(function()
            if Player and Player.Data then
                LevelText.Text = "📊 Level: " .. (Player.Data.Level.Value or 0)
                BeliText.Text = "💰 Beli: " .. (Player.Data.Beli.Value or 0)
                FragText.Text = "💎 Fragments: " .. (Player.Data.Fragments.Value or 0)
            end
        end)
    end
end)
...
function EquipWeapon()
    ...
    for _, src in pairs(sources) do
        for _, t in pairs(src:GetChildren()) do
            if t:IsA("Tool") then
                if t.ToolTip == "Blox Fruit" or t.ToolTip == "Sword" or t.ToolTip == "Melee" then
                    SelectWeapon = t.Name
...
local function UnlockColosseum()
    ...
    repeat
        FlyTo(bartiloPos.Position)
        task.wait(0.5)
    until (bartiloPos.Position - hrp.Position).Magnitude < 20
    ...
    for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
        if v.Name == "Toga Warrior" and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
            repeat
                ...
                StatusText.Text = "🤖 Status: Mở khóa đấu trường: " .. killed .. "/50 Toga Warrior"
...
local function CollectFlowers()
    local flowers = {"Flower1", "Flower2"}
    for _, flowerName in pairs(flowers) do
...
-- ========== NHIỆM VỤ TỘC V2 (LEVEL 1100) ==========
local function DoRaceV2()
    StatusText.Text = "🤖 Status: Đang làm nhiệm vụ tộc V2..."
    
    local alchemistPos = CFrame.new(-2779.83521, 72.966, -3574.020)
    repeat
        FlyTo(alchemistPos.Position)
        task.wait(0.5)
    until (alchemistPos.Position - hrp.Position).Magnitude < 20
...
-- ========== NHIỆM VỤ TỘC V3 (LEVEL 1500 - AROWE) ==========
local function DoRaceV3()
    StatusText.Text = "🤖 Status: Đang làm nhiệm vụ tộc V3..."
    
    local arowePos = CFrame.new(-5515.5, 788.5, -2835.5)
    repeat
        FlyTo(arowePos.Position)
        task.wait(0.5)
    until (arowePos.Position - hrp.Position).Magnitude < 20
    ...
    for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
        if v.Name == "Sky Bandit" and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
            repeat
                ...
                FlyAroundMob(v.HumanoidRootPart.Position)
                if v.Humanoid.Health <= 0 then killed = killed + 1 end
                StatusText.Text = "🤖 Status: Tộc V3 - Sky Bandit: " .. killed .. "/50"
            until v.Humanoid.Health <= 0
        end
    end
...
-- ========== NHIỆM VỤ CHUYỂN SEA 1 -> 2 (ICE ADMIRAL) ==========
local function DoQuestSea2()
    StatusText.Text = "🤖 Status: Đang làm nhiệm vụ chuyển Sea 2..."
    
    local icePos = CFrame.new(1347.7124, 37.375, -1325.6488)
    repeat
        FlyTo(icePos.Position)
        task.wait(0.5)
    until (icePos.Position - hrp.Position).Magnitude < 20
...
-- ========== NHIỆM VỤ CHUYỂN SEA 2 -> 3 (rip_indra) ==========
local function DoQuestSea3()
    StatusText.Text = "🤖 Status: Đang làm nhiệm vụ chuyển Sea 3..."
    
    local indraPos = CFrame.new(-26880.93359375, 22.848, 473.189)
    repeat
        FlyTo(indraPos.Position)
        task.wait(0.5)
    until (indraPos.Position - hrp.Position).Magnitude < 20
    
    for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
        if v.Name == "rip_indra" and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
            repeat
                task.wait()
                AutoHaki()
                EquipWeapon()
...
local function CheckAndDoQuests()
    ...
    -- Mở khóa đấu trường (level 850-900)
    if lvl >= 850 and lvl < 900 and game.PlaceId == 4442272183 then
        local bartiloProgress = game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BartiloQuest", "Progress")
    ...
    -- Tộc V2 (level 1100)
    if lvl >= 1100 and not isEvolved and game.PlaceId == 4442272183 then
        DoRaceV2()
    end
    
    -- Tộc V3 (level 1500)
    if lvl >= 1500 and isEvolved and game.PlaceId == 4442272183 then
        DoRaceV3()
    end
    
    -- Chuyển Sea 1 -> 2
    if lvl >= 700 and lvl < 710 and game.PlaceId == 2753915549 then
        DoQuestSea2()
    end
    
    -- Chuyển Sea 2 -> 3
    if lvl >= 1500 and lvl < 1510 and game.PlaceId == 4442272183 then
        DoQuestSea3()
    end
end
...
spawn(function()
    while task.wait() do
        pcall(function()
            -- KIỂM TRA VÀ LÀM NHIỆM VỤ (đấu trường, tộc V2/V3, chuyển sea)
            CheckAndDoQuests()
...
print("✅ AUTO FARM LEVEL - ĐÃ SẴN SÀNG! | anhlyhud")