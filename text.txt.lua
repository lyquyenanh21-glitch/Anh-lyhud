    ...
    for _, flowerName in pairs({"Flower1", "Flower2"}) do
        ...
        if flower and not plr.Backpack:FindFirstChild(flowerName) and not plr.Character:FindFirstChild(flowerName) then
            repeat
                ...
                task.wait(0.5)
                if (flower.Position - hrp.Position).Magnitude < 10 then firetouchinterest(hrp, flower, 0) task.wait(0.5) end
...
local function DoQuestSea2()
    StatusText.Text = "🤖 Status: Đang chuyển Sea 2..."
    local icePos = CFrame.new(1347.7124, 37.375, -1325.6488)
    repeat FlyTo(icePos.Position) task.wait(0.5) until (icePos.Position - hrp.Position).Magnitude < 20
    for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
        if v.Name == "Ice Admiral" and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
            repeat
                ...
                EquipWeapon()
                AttackNoCoolDown()
                pcall(function() v.HumanoidRootPart.Size = Vector3.new(60,60,60) v.HumanoidRootPart.CanCollide = false v.Humanoid.WalkSpeed = 0 end)
            until v.Humanoid.Health <= 0
        end
    end
    task.wait(2)
...
local function DoQuestSea3()
    StatusText.Text = "🤖 Status: Đang chuyển Sea 3..."
    local indraPos = CFrame.new(-26880.93359375, 22.848, 473.189)
    repeat FlyTo(indraPos.Position) task.wait(0.5) until (indraPos.Position - hrp.Position).Magnitude < 20
    for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
...
local function CheckAndDoQuests()
    ...
    if lvl >= 700 and lvl < 710 and game.PlaceId == 2753915549 then DoQuestSea2() end
    if lvl >= 1500 and lvl < 1510 and game.PlaceId == 4442272183 then DoQuestSea3() end
end
